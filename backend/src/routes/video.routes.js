...
router.get('/info', async (req, res) => {
  ...
  if (clientId)
    sendEvent(clientId, {
      ...
      progress: isSpotify ? 5 : 30
    });

  const spotifyData = isSpotify
    ? await resolveSpotifyToYoutube(
        videoURL,
        cookieArgs,
        (status, progress) => {
  ...
  if (isSpotify && clientId)
    sendEvent(clientId, { status: 'fetching_info', progress: 25 });
  ...
  try {
    ...
    console.log(
      `[Debug] All Format IDs found: ${info.formats
        .map(f => f.format_id)
        .join(', ')}`
    );
    ...
    const formats = info.formats
      .filter(f => {
        const hasVideo =
          (f.vcodec && f.vcodec !== 'none') || f.height || f.width;
      ...
      .map(f => {
        ...
        if (!h && f.resolution) {
          const m =
            f.resolution.match(/(\d+)p/i) || f.resolution.match(/x(\d+)/);
          if (m) h = parseInt(m[1]);
        }
  ...
  size = Math.floor((bitrateInBits * durationInSeconds) / 8);
    ...
    console.log(
      `[Info] Title: "${finalTitle}" | Cover: ${
        finalThumbnail ? 'Found' : 'Missing'
      }`
    );
...
router.all('/convert', async (req, res) => {
  ...
  const tempFilePath = path.join(
    TEMP_DIR,
    `${clientId}_${Date.now()}.${format}`
  );
  const coverPath = path.join(TEMP_DIR, `${clientId}_cover.jpg`);

  const sanitizedTitle =
    (finalMetadata.title || title).replace(/[<>:"/\\|?*]/g, '').trim() ||
    'video';
  ...
  try {
    ...
    if (finalMetadata.imageUrl) {
      try {
        if (finalMetadata.imageUrl.startsWith('data:image')) {
          // Handle Base64 Image
          const base64Data = finalMetadata.imageUrl.replace(
            /^data:image\/\w+;base64,/,
            ''
          );
          ...
          // Handle URL Download
          finalCoverPath = await downloadImage(
            finalMetadata.imageUrl,
            coverPath
          );
        }
      } catch (e) {
    ...
    videoProcess.stdout.on('data', data => {
      ...
      if (match && clientId) {
        sendEvent(clientId, {
          status: 'downloading',
          progress: Math.round(parseFloat(match[1]) * 0.95)
        });
      }
      if (
        clientId &&
        (output.includes('[Merger]') || output.includes('[ExtractAudio]'))
      ) {
    ...
    videoProcess.on('close', async code => {
      if (code === 0) {
        ...
        try {
          await injectMetadata(tempFilePath, {
            ...finalMetadata,
            coverFile: finalCoverPath
          });
        } catch (tagError) {
          console.warn(
            '[Metadata] Injection failed but continuing:',
            tagError.message
          );
        }
