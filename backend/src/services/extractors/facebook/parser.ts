...
function extractJsonObjects(source: string, extractedId?: string): string[] {
  ...
  for (const kw of keywords) {
    ...
    while ((pos = source.indexOf(kw, pos)) !== -1) {
      ...
      let foundCount = 0;
      while (
        (bracePos = source.lastIndexOf('{', bracePos - 1)) !== -1 &&
        bracePos >= limit
      ) {
...
function parseMuxedFormats(
  obj: string,
  extractedId: string,
  uniqueFormats: Map<string, Format>,
  options: { isStory?: boolean } = {}
): void {
  const hasTargetId = obj.includes(extractedId);
  const hasPlayable =
    obj.includes('playable_url') ||
    obj.includes('unified_video_url') ||
    obj.includes('base_url');

  if (!hasTargetId && !(options.isStory && hasPlayable)) return;

  const hdUrl = obj.match(
    /(?:playable_url_quality_hd|browser_native_hd_url|unified_video_url)[^:]*:\s*(?:\\)*"((?:\\.|[^"\\])+)/u
  )?.[1];
  const sdUrl = obj.match(
    /(?:playable_url|browser_native_sd_url|video_url|base_url)[^:]*:\s*(?:\\)*"((?:\\.|[^"\\])+)/u
  )?.[1];
  const audioUrl = obj.match(
    /"audioUrl"\s*:\s*(?:\\)*"((?:\\.|[^"\\])+)"/u
  )?.[1];
  ...
  const process = (url: string, id: string, label: string) => {
    ...
    const isAudioOnly =
      id === 'audio' ||
      decodedUrl.includes('audio_heaac') ||
      decodedUrl.includes('audio_only') ||
      decodedUrl.includes('.m4a');
    const isVideoOnly = decodedUrl.includes('bytestart');
    const hasAudio =
      isAudioOnly ||
      (!isVideoOnly &&
        (Boolean(audioUrl) || !decodedUrl.includes('bytestart')));
...
function getFallbackAuthor(fullSource: string, currentAuthor: string): string {
  ...
  const globalAuthorMatch = fullSource.match(
    /"name"\s*:\s*(?:\\)*"((?:\\.|[^"\\])+)"(?=.*?"__typename"\s*:\s*(?:\\)*"User")/u
  );
  return globalAuthorMatch ? decodeFull(globalAuthorMatch[1]) : currentAuthor;
}

function getStoryThumbnail(
  html: string,
  uniqueFormats: Map<string, Format>
): string | null {
...
function parseMetadata(
  obj: string,
  state: { author: string; finalTitle: string }
): void {
  // parse author
  const authorMatch = obj.match(
    /"(?:name|story_bucket_owner_name|story_bucket_owner)"\s*:\s*(?:\\)*"((?:\\.|[^"\\])+)"/u
  );
  const isUser =
    obj.includes('"User"') ||
    obj.includes('__typename') ||
    obj.includes('story_bucket_owner') ||
    obj.includes('story_bucket_owner_name');
  if (authorMatch && isUser && state.author === 'Facebook User') {
    const foundAuthor = decodeFull(authorMatch[1]);
    if (
      !foundAuthor.toLowerCase().includes('facebook') &&
      !foundAuthor.toLowerCase().includes('video')
    ) {
  ...
  // parse title
  const textMatch = obj.match(
    /(?:message|text|accessibility_caption)[^:]*:\s*(?:\\)*"((?:\\.|[^"\\]){5,500})/u
  );
...
function processThumbnail(
  ...
  const storyThumbnail = isStory
    ? getStoryThumbnail(html, uniqueFormats)
    : null;
  const cheerioDoc = load(html);
...
function getFinalFormats(uniqueFormats: Map<string, Format>): Format[] {
  ...
  if (hd && !formats.some((format) => format.formatId === 'hd'))
    hd.formatId = 'hd';
  if (sd && !formats.some((format) => format.formatId === 'sd'))
    sd.formatId = 'sd';
  return formats;
}
...
export function parseHtml(html: string, targetUrl: string): RawFacebookData {
  ...
  console.log(
    `[JS-FB] info: ${targetUrl} (ID: ${extractedId})${isStory ? ' (STORY)' : ''}${isReel ? ' (REEL)' : ''}`
  );

  const { ogTitle, ogDesc } = getOgMetadata(html);
  const cheerioDoc = load(html);
  const scriptsSet = cheerioDoc('script')
    .map((_i, el) => cheerioDoc(el).html())
    .get();
  ...
  const author = getFallbackAuthor(
    fullSource,
    ogTitle.includes(' | ') && state.author === 'Facebook User'
