...
const { fetchFromOdesli, fetchIsrcFromDeezer, fetchIsrcFromItunes } = require("./external");
    ...
    return new Promise(resolve => {
        ...
        const processResult = (result, c) => {
            ...
            onProgress("fetching_info", 85, {
                ...
                metadata_update: { title: metadata.title, artist: metadata.artist, cover: metadata.imageUrl, thumbnail: metadata.imageUrl }
            });
        ...
        candidates.forEach(c => {
            c.promise.then(result => processResult(result, c)).catch(() => {
                if (!isSettled) {
                    finishedCount++;
...
async function runPriorityRace(videoURL, metadata, cookieArgs, onProgress, soundchartsPromise = null) {
    ...
    if (cleanArtist) {
        const cleanPromise = (async () => {
            ...
            if (raceSettled) return null;
            return searchOnYoutube(`${metadata.title} ${cleanArtist}`, cookieArgs, metadata, early => safeProgress("fetching_info", 55, { metadata_update: { ...early, cover: metadata.imageUrl } }), false, signal);
        })();
        candidates.push({ type: "Clean", priority: 2, promise: cleanPromise });
    }

    const raceTimeout = setTimeout(() => { if (!raceSettled) raceController.abort("timeout"); }, 45000);
    ...
    try {
        ...
        if (isrcResult && (!bestMatch || (bestMatch.type !== "ISRC" && isrcResult.diff <= 2000))) return { ...isrcResult, type: "ISRC", priority: 0 };
        return bestMatch;
    } catch (err) {
        raceSettled = true;
