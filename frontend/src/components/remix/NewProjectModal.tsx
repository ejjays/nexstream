  ...
  return (
    <div className="bg-[#0a0a0a] border border-[#22d3ee]/20 rounded-2xl p-6 space-y-4 animate-in fade-in zoom-in-95 duration-200">
      ...
      <p className="text-[10px] text-zinc-600 leading-relaxed italic">
        * Your keys are only used to trigger the Kaggle kernel via your own
        account and are never stored on our servers.
      </p>
    </div>
  );
};
...
const EngineButton = ({
  ...
  >
    <div className="flex items-center justify-between w-full mb-2">
      <span
        className={`text-lg font-bold ${isActive ? 'text-[#22d3ee]' : 'text-white'}`}
      >
        {label}
      </span>
      {isActive && (
...
const NewProjectModal = ({
  ...
  const [kaggleUser, setKaggleUser] = useState(
    () => localStorage.getItem('kaggle_user') || ''
  );
  const [kaggleKey, setKaggleKey] = useState(
    () => localStorage.getItem('kaggle_key') || ''
  );
    ...
    const poll = async () => {
      try {
        const res = await fetch(
          `${getBackendUrl()}/api/remix/engine-status?session_id=${currentSessionId}`
        );
        const data = await res.json();
