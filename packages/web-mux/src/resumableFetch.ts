...
export class SizeMismatchError extends Error {
  constructor(message: string) {
    super(message);
    this.name = 'MuxResumeMismatch';
  }
}

export class FetchIncompleteError extends Error {
  constructor(message: string) {
    super(message);
    this.name = 'FetchIncomplete';
  }
}
...
// fetches to a sink with byte-range resume on drop; rejects if size changes mid-resume
export async function resumableFetchToSink(
  ...
  const giveUp = (): never => {
    if (state.total > 0 && state.offset < state.total) {
      throw new FetchIncompleteError(
