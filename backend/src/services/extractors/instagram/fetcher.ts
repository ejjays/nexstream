...
export async function fetchOembed(
  ...
  try {
    const res = await secureFetch(oembedUrl, {
    ...
    if (res.ok) {
      ...
      console.debug(
        '[InstagramFetcher] OEmbed schema validation failed:',
        parsed.error.message
      );
    } else {
      console.debug(
        `[InstagramFetcher] OEmbed fetch failed with status: ${res.status}`
      );
    }
  } catch (e: unknown) {
...
export async function fetchGraphql(
  ...
  const res = await secureFetch(gqlUrl, {
    headers: fetchHeaders,
  ...
  if (res.ok) {
    ...
    if (parsed.success) return parsed.data;
    console.debug(
      '[InstagramFetcher] GraphQL schema validation failed:',
      parsed.error.message
    );
  }
  return null;
