  ...
  const supportingStack = [
    {
      ...
      items: [
        { name: "Groq & Gemini", desc: "Multi-model AI reasoning for high-precision metadata resolution." },
        { name: "Deezer/iTunes", desc: "Authoritative ISRC verification and high-fidelity asset mapping." }
      ]
    },
    {
      ...
      items: [
        { name: "Turso (libSQL)", desc: "Edge-hosted persistent registry for global track indexing." },
        { name: "Odesli API", desc: "Industry-standard link resolution and cross-platform bridge logic." }
      ]
    },
    {
      category: "Interface",
      ...
      bgColor: "bg-blue-500",
      items: [
        { name: "React 19", desc: "Modern declarative UI with high-performance concurrent rendering." },
        { name: "Framer Motion", desc: "GPU-accelerated orchestration for complex interface transitions." }
      ]
    },
    {
      ...
      items: [
        { name: "Node.js Streams", desc: "Advanced memory piping protocol for zero-disk asset delivery." },
        { name: "Reactive Pulse", desc: "Server-Sent Events (SSE) for live backend telemetry streaming." }
      ]
    }
  ];
    ...
    <div className='w-full flex flex-col gap-12 pb-12'>
      ...
      <section className='space-y-8'>
        <div className='flex items-center gap-3 px-4'>
          ...
          <h2 className='text-white font-black uppercase tracking-[0.2em] text-xs'>Core Engines</h2>
        </div>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-8'>
          <GlassCard className='group relative overflow-hidden p-8 border-cyan-500/20 bg-cyan-500/5'>
            ...
            <div className='relative z-10 space-y-4'>
              <div className='text-cyan-400 text-2xl font-black uppercase tracking-tighter'>yt-dlp</div>
              <p className='text-gray-300 text-sm leading-relaxed max-sm:text-xs max-w-sm'>
                The world's most advanced media manifest resolver. It provides the low-level resolution required to bypass restrictions and establish high-speed source connections.
              </p>
          ...
          <GlassCard className='group relative overflow-hidden p-8 border-purple-500/20 bg-purple-500/5'>
            ...
            <div className='relative z-10 space-y-4'>
              <div className='text-purple-400 text-2xl font-black uppercase tracking-tighter'>FFmpeg 8.0</div>
              <p className='text-gray-300 text-sm leading-relaxed max-sm:text-xs max-w-sm'>
                The standard multimedia framework. Every byte of media processed by NexStream passes through our custom memory pipes for frame-perfect muxing.
              </p>
      ...
      <section className='space-y-8'>
        <div className='flex items-center gap-3 px-4'>
          ...
          <h2 className='text-gray-500 font-black uppercase tracking-[0.2em] text-xs'>Supporting Ecosystem</h2>
        </div>
        
        <div className='grid grid-cols-1 md:grid-cols-2 gap-8'>
          {supportingStack.map((group, idx) => (
            <GlassCard key={idx} className="group relative overflow-hidden">
              <div className={`absolute inset-y-0 left-0 w-1 ${group.bgColor} opacity-40 group-hover:opacity-100 group-hover:w-1.5 transition-all duration-500`} />
                ...
                <div className='grid gap-6'>
                  {group.items.map((item, iIdx) => (
                    <div key={iIdx} className='space-y-1.5'>
                      <div className={`${group.textColor} text-xs font-black uppercase tracking-wide`}>
                        {item.name}
                      </div>
        ...
        <div className='relative p-12 rounded-[3rem] bg-[#020617]/60 backdrop-blur-3xl border border-cyan-500/20 text-center overflow-hidden isolation-auto transform-gpu'>
          ...
          <div className='relative z-10 space-y-6'>
            <div className='inline-flex items-center gap-3 px-4 py-1.5 rounded-full bg-cyan-400/10 border border-cyan-400/20 text-cyan-300 font-black uppercase tracking-[0.4em] text-[9px] mb-2'>
              <Heart size={12} fill="currentColor" className="text-cyan-400 animate-pulse" /> A Quiet Note of Gratitude
            </div>
            <p className='text-lg text-cyan-50 leading-relaxed max-w-2xl mx-auto font-medium'>
              NexStream exists purely as a bridge to the powerful engines that drive the modern web. The real heavy lifting is done by the legendary contributors behind <span className='text-white font-black underline decoration-cyan-400 decoration-2 underline-offset-8 bg-cyan-400/10 px-2 py-0.5 rounded-md'>yt-dlp</span> and <span className='text-white font-black underline decoration-cyan-400 decoration-2 underline-offset-8 bg-cyan-400/10 px-2 py-0.5 rounded-md'>FFmpeg</span>. Sincere respect to the maintainers who dedicate their time to keeping these tools free and open for everyone.
            </p>
            <div className='flex flex-col items-center gap-2 mt-8 opacity-60'>
              ...
              <p className='text-cyan-400/60 text-[10px] font-black uppercase tracking-[0.5em]'>
                Respecting the silent giants of open-source
              </p>
            </div>
          </div>
        </div>
