...
const MusicPlayerCard = (
  ...
  const playerContent = (<AnimatePresence>
    {isVisible && (<motion.div
      ...
      <div className="relative group w-80 outline-none">
        <div
          className="absolute -inset-0.5 bg-gradient-to-r from-cyan-500 to-purple-600 rounded-[1.5rem] blur opacity-25 group-hover:opacity-45 transition duration-1000"></div>
        <div
          className="relative w-full rounded-[1.5rem] bg-black/75 backdrop-blur-[45px] backdrop-saturate-[150%] border border-white/10 shadow-2xl overflow-hidden outline-none">
          <div
            className="absolute inset-0 bg-gradient-to-tr from-white/10 via-transparent to-white/5 pointer-events-none z-20" />
          <div
            className="relative w-full p-3.5 pt-6 transition-colors duration-500 hover:bg-white/[0.02] outline-none">
            <div
            ...
            <div
              className="absolute top-2.5 left-1/2 -translate-x-1/2 w-10 h-1 bg-white/20 rounded-full group-hover:bg-white/40 transition-all duration-500" />
            <button
              onClick={onClose}
            ...
            <div className="flex items-center gap-4 relative z-10">
              ...
              <div className="flex-1 min-w-0">
                <h4
                  className="text-white text-[13px] font-bold truncate tracking-tight mb-0.5">{data?.title || "Unknown Title"}</h4>
                <p
                  className="text-cyan-400 font-black text-[9px] truncate uppercase tracking-[0.25em]">{data?.artist || "Unknown Artist"}</p>
            ...
            <div
              className="mt-3 flex items-center justify-between pt-2 border-t border-white/10 pl-1 pr-2">
              <div className="flex items-center gap-2">
                <Music2
                  size={10}
                  className={`${hasPreview ? "text-purple-400 drop-shadow-[0_0_5px_rgba(168,85,247,0.5)] animate-pulse" : "text-white/20"}`} />
                <span
                  className="text-[8px] uppercase tracking-[0.4em] text-white/40 font-black">{hasPreview ? "Spotify Music Preview" : "Preview Unavailable"}</span>
              </div>
              {hasPreview && (<span
                className="text-[7px] text-cyan-400 font-black tracking-widest uppercase border border-cyan-500/30 px-1.5 py-0.5 rounded bg-cyan-500/10 shadow-[0_0_10px_rgba(6,182,212,0.2)]">Space</span>)}
            </div>
          </div>
        </div>
      </div>
