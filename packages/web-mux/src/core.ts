...
import { shouldVetoCopyMux, UnsupportedMuxCodecError } from './codecs.js';
...
export async function pumpTrack(
  ...
  while (packet) {
    if (signal?.aborted) throw new Error('muxing aborted');
    const shifted =
      offset > 0
...
// fast remux (no re-encode) to fragmented mp4
