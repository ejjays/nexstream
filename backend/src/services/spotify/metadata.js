    ...
    try {
        ...
        if (!details) {
            try {
                ...
                if (oembedData) {
                    details = { name: oembedData.title, artists: [{ name: "Unknown Artist" }] };
                }
            } catch (e) {}
        }
...
async function fetchInitialMetadata(videoURL, onProgress, startTime) {
    ...
    onProgress("fetching_info", 20, {
        ...
        metadata_update: {
            title: firstMetadata.title, artist: firstMetadata.artist,
            cover: firstMetadata.imageUrl, thumbnail: firstMetadata.imageUrl,
