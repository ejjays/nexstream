...
export function processVideoFormats(info: { duration?: number, formats?: RawFormat[]; streaming_data?: { formats?: RawFormat[]; adaptive_formats?: RawFormat[] } }): Format[] {
  ...
  for (const f of processed) {
    ...
    if (!existing) {
      ...
      // If both are muxed or both are video-only, prefer the one with a larger filesize (higher quality/bitrate)
      else if (f.is_muxed === existing.is_muxed && f.filesize > existing.filesize) {
        uniqueFormats.set(key, f);
      }
    }
  }
