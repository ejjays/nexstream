...
const FormatGuide = () => {
  ...
  const features = [{
    ...
    bgColor: "bg-cyan-400",
    description: "The Gold Standard. Crystal-clear audio that works on every device without losing a single bit.",
  ...
  return (
    <div className="w-full flex flex-col gap-10">
      ...
      <header className="text-center flex flex-col items-center gap-4">        <div
        ...
        <h1
          className="text-4xl md:text-6xl font-black uppercase tracking-tighter text-white">Audio <span className="text-cyan-400">Format Guide</span>
        </h1>
        <p className="text-gray-400 max-w-xl font-medium">The Science of Perfect Audio. Choose your priority: Speed or Quality.
                  </p>
      </header>
      <section
        className="bg-white/5 backdrop-blur-xl border border-white/10 p-8 rounded-[2rem] shadow-2xl relative overflow-hidden group">
        <div
          className="absolute -top-24 -right-24 w-48 h-48 bg-cyan-500/10 blur-[80px] group-hover:bg-cyan-500/20 transition-all duration-700"></div>
        <h2
          className="text-2xl font-black text-cyan-400 mb-4 flex items-center gap-3 uppercase tracking-tighter">🚀 Two Engines, One App
                  </h2>
        <p className="text-gray-300 leading-relaxed text-lg">
          <span className="text-white font-bold">NexStream</span>features a
                    Hybrid Engine. Use our{" "}
          <span
            className="text-emerald-400 font-bold underline decoration-emerald-500/30 underline-offset-4">Lightning Engine
                      </span>{" "}for instant MP3s, or our{" "}
          <span
            className="text-cyan-400 font-bold underline decoration-cyan-500/30 underline-offset-4">Direct-Stream Engine
                      </span>{" "}for lossless M4A/WebM files that come straight from the master servers
                    without re-encoding.
                  </p>
      </section>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {features.map(
          (feature, idx) => (<GlassCard key={idx} className="group relative overflow-hidden">
            {}
            <div
            ...
            <div className="p-8 flex flex-col h-full">
              ...
              <ul className="space-y-4 mt-auto">
                {feature.points.map((point, pIdx) => (<li
                  key={pIdx}
                  ...
                  <span className={`${point.bold ? "text-white font-bold" : ""}`}>
                    {point.text}
                  </span>
                </li>))}
              </ul>
      ...
      <section
        className="bg-black/40 backdrop-blur-xl border border-cyan-500/20 p-8 rounded-[2rem] shadow-inner">
        <div className="flex items-center gap-3 mb-6">
          <span className="text-2xl">🔬</span>
          <h2 className="text-xl font-black text-cyan-400 uppercase tracking-tighter">Speed vs Quality: The Tech
                      </h2>
        </div>
        <div className="space-y-6 text-sm text-gray-400 leading-relaxed">
          <p>
            <span className="text-white font-bold">MP3 (Speed Mode)</span>uses{" "}
            <span className="text-emerald-400 font-bold">High-Fidelity Real-Time Transcoding
                          </span>. We decode the source and immediately re-encode it into a
          ...
          <p>
            <span className="text-white font-bold">Direct-Stream Copy (M4A/WebM)
                          </span>{" "}takes longer to initialize (~5s) because we use elite algorithms to
      ...
      <footer className="flex flex-col items-center gap-8 mt-12 pb-12">
        <div className="space-y-4 text-center">
          <div className="space-y-1 text-gray-500 text-sm">
            <p>For instant downloads & universal support, choose{" "}
              <span className="text-emerald-400 font-bold">MP3</span>.
                          </p>
            <p>For maximum quality & rich metadata, choose{" "}
              <span className="text-cyan-400 font-bold">M4A</span>or{" "}
              <span className="text-amber-400 font-bold">WebM</span>.
                          </p>
          </div>
          <p
            className="text-sm text-cyan-400 font-black uppercase tracking-widest pt-4 border-t border-white/5">God bless & thank you for using NexStream.
                      </p>
        </div>
        <div className="flex items-center justify-center gap-4">
          <button
            onClick={() => window.close()}
            className="flex items-center gap-2 font-mono text-[10px] uppercase tracking-widest border border-white/10 px-8 py-3 rounded-full hover:bg-white/10 hover:border-white/20 transition-all duration-300 font-black text-gray-400">Close Guide
                      </button>
        </div>
      </footer>
    </div>
