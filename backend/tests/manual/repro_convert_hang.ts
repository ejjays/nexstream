...
async function testPerformance(): Promise<void> {
  ...
  try {
    ...
    const streamRes = await axios.get(
      `${baseUrl}/stream-urls?url=${encodeURIComponent(youtubeUrl)}&id=test-perf&formatId=299`
    );
    ...
    response.data.on('data', (chunk: Buffer) => {
      ...
      if (elapsedSinceLog >= 3) {
        ...
        console.log(
          `[${Math.round((now - start) / 1000)}s] Total: ${(totalBytes / (1024 * 1024)).toFixed(2)} MB | Speed: ${speedMBps.toFixed(2)} MB/s`
        );
