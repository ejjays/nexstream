...
async function measureSpeed(): Promise<void> {
  ...
  try {
    ...
    const streamRes = await axios.get(
      `${baseUrl}/stream-urls?url=${encodeURIComponent(youtubeUrl)}&id=test-speed&formatId=251`
    );
    ...
    const timer = setInterval(() => {
      ...
      console.log(
        `[${Math.round((now - start) / 1000)}s] Speed: ${speedMB.toFixed(2)} MB/s | Total Received: ${totalMB} MB`
      );
