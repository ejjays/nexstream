      ...
      >
        ...
        <div className="flex justify-center items-center mb-2">
          <h3 className="text-xl font-bold flex items-center gap-2.5">
            <Metronome
              size={24}
              className={isMetronome ? 'text-cyan-400' : 'text-zinc-500'}
            />
            Smart Metronome
          </h3>
        </div>
        ...
        <div className="flex items-center justify-between bg-zinc-800/40 p-5 rounded-2xl mb-6 border border-zinc-800/50">
          <div className="flex flex-col">
            ...
            <span className="text-xs text-zinc-500">
              Perfect beat-sync synchronization
            </span>
          </div>
          <button
          ...
          >
            <span
              className={`inline-block h-5 w-5 transform rounded-full bg-white transition-transform ${isMetronome ? 'translate-x-6' : 'translate-x-1'}`}
            />
          </button>
        </div>

        <div
          className={`transition-all duration-300 ${isMetronome ? 'opacity-100' : 'opacity-40 pointer-events-none'}`}
        >
          ...
          <span className="text-xs text-zinc-400 font-bold uppercase tracking-widest mb-4 block px-1">
            Sound Signature
          </span>
          ...
          <div className="px-1 border-t border-white/5 pt-6">
            <div className="flex justify-between items-center mb-3">
              ...
              <span
                className={`text-xs font-mono font-bold ${gridShift === 0 ? 'text-zinc-500' : 'text-cyan-400'}`}
              >
                {gridShift > 0 ? '+' : ''}
                {gridShift} Beats
              </span>
            ...
            <p className="text-[10px] text-zinc-600 mt-2 text-center italic">
              Nudge chord labels left or right to match the audio
            </p>
          </div>
        </div>
      </div>
