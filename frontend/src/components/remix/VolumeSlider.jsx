...
const VolumeSlider = ({ track, initialVolume, onVolumeChange, onVolumeCommit }) => {
  ...
  // Sync if parent updates (like reset)
  useEffect(() => {
    if (!isPointerDown.current) {
      updateDOM(initialVolume, true); // Glide on reset
  ...
  const handleInput = (e) => {
    ...
    // Only snap-update the DOM if we are actually dragging
    if (didMove.current) {
      updateDOM(val, false);
    }
  ...
  const handlePointerUp = (e) => {
    ...
    if (!didMove.current) {
      // It was a tap! Trigger the smooth animation to the new value
  ...
  return (
    <div className='flex items-center gap-3 sm:gap-6 group'>
      ...
      <div className='flex-1 relative flex items-center h-8'>
        ...
        {/* The actual invisible input slider handling the math/drags */}
        <input
          ref={inputRef}
          type='range'
          min='0'
