...
const Tuner = ({ onClose }: TunerProps) => {
  const { note, cents, isRecording, isLoadingModel, error, start, stop } =
    useTuner();
      ...
      <div className="bg-[#141414] border border-white/5 rounded-3xl w-full max-w-md shadow-2xl flex flex-col overflow-hidden">
        <div className="flex items-center justify-between p-5 border-b border-white/5 bg-[#0a0a0a]">
          <div className="flex items-center gap-3">
            <div
              className={`w-2.5 h-2.5 rounded-full ${isRecording && !isLoadingModel ? 'bg-red-500 animate-pulse' : 'bg-zinc-600'}`}
            ></div>
          ...
          <button
            onClick={onClose}
            className="text-zinc-500 hover:text-white p-2 transition-colors -mr-2"
          >
        ...
        <div className="p-8 flex flex-col items-center">
          {error ? (
            ...
            <>
              <div
                className="text-[120px] font-black leading-none mb-2 tracking-tighter"
                style={{
                  color: isTuned && note !== '-' ? '#22d3ee' : '#ffffff',
                  textShadow:
                    isTuned && note !== '-'
              ...
              <div
                ...
                style={{
                  color: isTuned && note !== '-' ? '#22d3ee' : '#a1a1aa',
                }}
              >
                {note === '-'
                  ? 'Waiting for audio...'
