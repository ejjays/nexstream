...
async function priorityRace(candidates, metadata, onProgress, getElapsed, settleCallback = () => {}) {
    return new Promise((resolve) => {
        ...
        const processResult = (result, c) => {
            ...
            if (!isGoodMatch) {
                console.log(`[Quantum Race] Engine ${c.type} rejected: Drift too high (${(result.diff/1000).toFixed(1)}s)`);
                if (finishedCount === candidates.length) { settle(bestMatch, 'All finished'); } return;
            }
            ...
            if (c.priority === 0) {
                ...
                const waitTime = calculateWaitTime(candidates.some(cand => cand.priority === 0), metadata.duration > 0 && result.diff < 2000, c.priority);
                if (graceTimer) { clearTimeout(graceTimer); }
    ...
    try {
        ...
        if (brainData.formats?.length) {
            onProgress('fetching_info', 95, { 
                ...
                metadata_update: { 
                    title: brainData.title, artist: brainData.artist, cover: brainData.imageUrl, thumbnail: brainData.imageUrl,
                    duration: brainData.duration / 1000, previewUrl: brainData.previewUrl, formats: brainData.formats, audioFormats: brainData.audioFormats, isFullData: true 
                } 
            });
...
async function fetchInitialMetadata(videoURL, onProgress, startTime) {
    ...
    const scrapersPromise = fetchFromScrapers(videoURL).then(res => {
        if (res?.previewUrl && onProgress) {
            ...
            onProgress('fetching_info', 20, { metadata_update: { previewUrl: res.previewUrl } });
        }
        return res;
    ...
    onProgress('fetching_info', 20, { 
        subStatus: 'Metadata locked.', details: `IDENTITY: "${firstMetadata.title.toUpperCase()}"`, 
        metadata_update: { 
            title: firstMetadata.title, artist: firstMetadata.artist, cover: firstMetadata.imageUrl, thumbnail: firstMetadata.imageUrl,
            duration: firstMetadata.duration / 1000, previewUrl: firstMetadata.previewUrl 
        } 
    });
