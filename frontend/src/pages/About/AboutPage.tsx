...
const HeroSection = ({ variants }: { variants: Variants }) => (
  <motion.section variants={variants} className="text-center space-y-4 flex flex-col items-center">
    <motion.div
      variants={variants}
      animate={{ rotate: 360 }}
...
const SupportCard = () => (
  <div className="md:col-span-2 space-y-6 bg-black/20 backdrop-blur-md border border-white/5 p-6 rounded-[2rem]">
    ...
    <p className="text-xs text-gray-400 leading-relaxed">Your support helps me keep the servers running and allows me to stay focused on continue developing the next generation of open-source media tools.</p>
    <button
      onClick={() => {
...
const SocialLinks = ({ variants, links }: { variants: Variants; links: SocialLinkItem[] }) => (
  <motion.section variants={variants} className="flex flex-col items-center gap-4">
    <div className="sm:hidden"><SocialMedia /></div>
    <div className="hidden sm:flex flex-wrap justify-center gap-4">
      {links.map((social) => (
        ...
        >
          ...
          <span className="text-white text-[11px] font-black uppercase tracking-widest transition-colors group-hover:text-cyan-400">{social.name}</span>
        </a>
      ))}
    </div>
  </motion.section>
...
const AboutPage = () => {
  ...
  const values: ValueItem[] = [
    { icon: <Heart className="text-rose-400" />, title: "Free for Everyone", text: "I believe high quality tools should never be hidden behind paywalls or annoying ads." },
  ...
  return (
    <motion.div key={pathname} variants={containerVariants} initial="hidden" animate="visible" className="w-full flex flex-col gap-12">
      <SEO title="Our Story | The NexStream Mission" description="Born out of frustration with bloatware and malware, NexStream was built to ensure high-quality media extraction remains free, private, and accessible to everyone." canonicalUrl="/resources/story" />
      ...
      <motion.section variants={itemVariants} className="space-y-12">
        <h2 className="text-center text-sm font-black text-gray-500 uppercase tracking-[0.4em]">Technical Foundation</h2>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-8">
          {techStack.map((t) => <TechCard key={t.name} t={t} />)}
        </div>
      </motion.section>
