...
function downloadCookiesBackground(
  type: string,
  cookieUrl: string,
  cookiesPath: string
): Promise<string | null> {
    ...
    https
      .get(cookieUrl, (response) => {
        if (response.statusCode !== 200) {
          isValidCookieFile(cookiesPath).then((valid) =>
            resolve(valid ? cookiesPath : null)
          );
          return;
        }
    ...
    if (!cachedTs || now - cachedTs > CACHE_DURATION) {
      (async () => {
        try {
          ...
          if (cookieUrl)
            await downloadCookiesBackground(type, cookieUrl, cookiesPath);
        } catch (e: unknown) {
          console.debug(
            '[CookieUtil] DB fetch error in background:',
            (e as Error).message
          );
        }
      })().catch((err) =>
        console.error(`[CookieUtil] Background sync failed: ${err.message}`)
      );
    }
    return cookiesPath;
  }
