...
const LogLine = ({ log, isLast = false, isTyping = false }) => {
  ...
  return (
    ...
    >
      ...
      <span
        className={`shrink-0 opacity-50 group-hover/item:opacity-100 transition-opacity w-3 text-center font-black ${getLogColor(log.type)}`}
      >
        {getLogSymbol(log.type)}
      </span>

      <span
        className={`break-words tracking-tight flex-1 relative ${getTextColor(log.type)}`}
      >
