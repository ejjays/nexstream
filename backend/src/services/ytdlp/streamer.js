...
function handleMp3Stream(url, formatId, cookieArgs, preFetchedInfo) {
    ...
    const proxy = {
        ...
        kill: () => { if (ffmpegProcess?.exitCode === null) ffmpegProcess.kill("SIGKILL"); },
    ...
    (async () => {
        try {
            ...
            ffmpegProcess = spawn("ffmpeg", [
                "-hide_banner", "-loglevel", "error", "-reconnect", "1", "-reconnect_streamed", "1", "-reconnect_delay_max", "5",
                "-user_agent", USER_AGENT,
                ...(referer ? ["-referer", referer] : []),
                ...(cookieString ? ["-cookies", cookieString] : []),
                "-i", audioFormat.url, "-c:a", "libmp3lame", "-b:a", "192k", "-f", "mp3", "pipe:1"
            ]);
            ffmpegProcess.stdout.pipe(combinedStdout);
            ffmpegProcess.on("close", code => eventBus.emit("close", code));
        } catch (err) {
            combinedStdout.emit("error", err); eventBus.emit("close", 1);
        }
    })();
    return proxy;
}
...
function handleDoublePipeStream(url, formatId, cookieArgs, combinedStdout, eventBus, proxy) {
    const ytdlpProc = spawn("yt-dlp", [
        ...
        "-f", formatId || "best", "-o", "-", url
    ]);
    const ffmpegProc = spawn("ffmpeg", [
        "-hide_banner", "-loglevel", "error", "-i", "pipe:0", "-c", "copy", "-bsf:a", "aac_adtstoasc",
...
function handleVideoStream(url, formatId, cookieArgs, preFetchedInfo) {
    ...
    (async () => {
        try {
            ...
            const ffmpegArgs = [
                "-hide_banner", "-loglevel", "error", ...ffmpegInputs, "-c", "copy", "-bsf:a", "aac_adtstoasc",
                "-map", "0:v:0", ...audioMap, "-shortest", "-f", "mp4", "-movflags", "frag_keyframe+empty_moov+default_base_moof", "pipe:1"
            ];
