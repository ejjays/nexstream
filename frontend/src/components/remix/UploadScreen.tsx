...
import { Loader2, Plus, Search, ChevronRight, Radio } from 'lucide-react';
...
const SongLibrary = ({
  ...
  <div className="flex-1 overflow-y-auto px-4 sm:px-8 py-2 flex flex-col">
      ...
      <div className="flex flex-col gap-1 mb-4">
        {history.slice(0, visibleCount).map((item, idx) => (
          <SongItem
            ...
            onDelete={(i: ProjectItem) =>
              setDeleteModal({ isOpen: true, id: i.id, projectName: i.name })
            }
          />
        ))}
    ...
    <div className="mt-2 border-t border-zinc-900/50">
      <h3 className="text-sm font-bold text-zinc-500 tracking-widest mb-4 mt-4">
        Try these Examples
      </h3>
      <div className="flex flex-col gap-1 mb-4">
        {DEMO_SONGS.map((demo) => (
  ...
  if (isProcessing) {
    return (
      <div className="fixed inset-0 bg-[#050505]/95 backdrop-blur-sm z-[150] flex flex-col items-center justify-center">
        <Loader2
          className="text-[#22d3ee] animate-spin mb-4"
          size={48}
          strokeWidth={2}
        />
    ...
    <div className="flex-1 flex w-full bg-black h-full absolute inset-0 z-[100] overflow-hidden text-white font-sans">
      ...
      <RenameModal
        ...
        setNewName={(val: string) =>
          setRenameModal((prev) => ({ ...prev, newName: val }))
        }
        onCancel={() =>
          setRenameModal({
            isOpen: false,
