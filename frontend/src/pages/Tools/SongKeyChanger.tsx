...
const ReadyState = ({ file, detectedInfo, reset, originalKey, setOriginalKey, targetKey, setTargetKey, semitones, startConversion }: any) => (
  <motion.div
    initial={{ opacity: 0, y: 20 }}
    animate={{ opacity: 1, y: 0 }}
    className='w-full'
...
const ResultState = ({ file, originalKey, targetKey, downloadUrl, audioProgress, isPlaying, togglePlayback, handleTimeUpdate, reset, resultAudioRef }: any) => (
  <motion.div
    initial={{ opacity: 0, y: 20 }}
    animate={{ opacity: 1, y: 0 }}
    ...
    <div className='flex flex-col min-h-screen w-full relative overflow-hidden text-slate-200 font-sans selection:bg-cyan-500/30'>
      <SEO 
        ...
        schema={{
          ...
          "description": "Detect or transpose your tracks into any key with high-quality audio processing."
        }}
      />
      <DotPattern />
