...
function handleDoublePipeStream(url, formatId, cookieArgs, combinedStdout, eventBus, proxy) {
    ...
    proxy.kill = () => {
        if (ytdlpProc.exitCode === null)
            ytdlpProc.kill("SIGKILL");if (ffmpegProc.exitCode === null)
...
function streamDownload(url, options, cookieArgs = [], preFetchedInfo = null) {
    ...
    if (["m4a", "webm", "audio", "opus"].includes(format)) return spawn(
        "yt-dlp",
        ["-f", formatId || "bestaudio[ext=m4a]/bestaudio", "-o", "-", ...baseArgs]
    );
...
async function injectMetadata(filePath, metadata) {
    return new Promise(resolve => {
        ...
        ["title", "artist", "album"].forEach(k => {
            if (metadata[k])
                ffmpegArgs.push("-metadata", `${k}=${metadata[k]}`);
        });
        ...
        ff.on("close", code => {
            ...
            if (fs.existsSync(tempOut))
                fs.unlinkSync(tempOut);resolve(false);
        });
    });
}
