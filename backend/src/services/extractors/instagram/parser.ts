...
function extractCaption(html: string, jsonData: Record<string, unknown> | null, $: CheerioAPI): string {
    ...
    // search scripts
    const captionMatch = html.match(/"edge_media_to_caption":\s*\{\s*"edges":\s*\[\s*\{\s*"node":\s*\{\s*"text":\s*"([^"]+)"/);
    if (captionMatch) return captionMatch[1];

    return '';
}

export function parseEmbed(html: string, existing: RawInstagramData): RawInstagramData {
    const newData = { ...existing };
    ...
    try {
        const scriptContent = cheerioDoc('script').filter((_i, el) => {
            ...
            return text.includes('window.__additionalDataLoaded') || text.includes('_sharedData');
        }).first().text();
    ...
    const videoUrl = jsonData?.shortcode_media?.video_url || 
                     html.match(/"video_url":\s*"([^"]+)"/)?.[1] ||
                     cheerioDoc('meta[property="og:video"]').attr('content');

    const displayUrl = jsonData?.shortcode_media?.display_url ||
