...
exports.processVideoFormats = info => {
  ...
  }).filter(f => f.height > 0 || f.quality !== "Unknown").sort((a, b) => b.height - a.height);
...
function getAudioQuality(f) {
  if (f.abr)
    return `${Math.round(f.abr)}kbps`;
  if (f.tbr && (!f.vcodec || f.vcodec === "none"))
    return `${Math.round(f.tbr)}kbps`;
  if (f.format_note && f.format_note.includes("kbps"))
...
exports.processAudioFormats = info => {
  ...
  const rawAudio = info.formats.filter(
