...
export async function fetchIsrcFromDeezer(
  ...
  try {
    ...
    let searchData: any = await searchDeezer(`artist:"${artist}" track:"${title}"`);
...
export async function fetchFromOdesli(spotifyUrl: string): Promise<any> {
  ...
  try {
    ...
    const spotifyEntity: any = Object.values(data.entitiesByUniqueId).find((e: any) => e.platforms?.includes('spotify'));
