...
const getInitialOptions = (selectedFormat = 'mp3', videoData = {}) => {
  try {
    ...
    // Total isolation of data extraction
...
const QualitySelection = ({ 
  ...
  <div className='space-y-2 mt-2 relative'>
    <p className='text-cyan-400 text-[10px] font-black uppercase tracking-[0.15em] ml-1 opacity-80'>
      {isPartial ? 'Searching for best quality streams...' : 'Select Output Quality'}
    </p>
    <div className='flex gap-2.5 relative'>
            ...
            <>
              ...
              >
                <div className='flex flex-col'>
                  ...
                  <span className='text-[10px] text-cyan-400/60 font-medium mt-0.5'>{formatSize(selectedOption?.filesize)} • {selectedOption?.extension?.toUpperCase() || 'RAW'}</span>
                </div>
                <ChevronDown className={`text-gray-400 transition-all duration-500 ${isDropdownOpen ? 'rotate-180 text-cyan-400 scale-110' : 'group-hover:text-white'}`} size={20} />
              </motion.button>
                    ...
                    <div className='max-h-60 overflow-y-auto custom-scrollbar mb-4 mt-1 mx-1.5 py-1'>
                      {options.map((option, idx) => (
                        ...
                        >
                          ...
                          {selectedQualityId === option.format_id && (
                            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className='bg-cyan-500/20 p-1 rounded-full relative z-10'><Check size={12} strokeWidth={4} /></motion.div>
                          )}
                        </button>
                      ))}
                    </div>
      ...
      <motion.button
        ...
        className={`${isPartial ? 'bg-gray-800 border-gray-700 cursor-not-allowed text-gray-500' : 'bg-cyan-500 hover:bg-cyan-400 text-white shadow-[0_10px_20px_rgba(6,182,212,0.3)] border-cyan-400/30'} px-7 py-3 rounded-2xl flex items-center gap-2 font-black transition-all border shrink-0`}
      >
        {isPartial ? (
          <motion.div
            animate={{ rotate: 360 }}
...
const EditModeUI = ({ 
  ...
  selectedFormat, videoData, setIsEditing 
}) => (
    ...
    <div className='flex flex-col gap-3'>
      ...
      <div className='flex gap-3 items-center'>
        <div className='space-y-1 flex-1'>
          <label className='text-[10px] text-cyan-400 uppercase font-bold tracking-wider ml-1'>{selectedFormat === 'mp4' ? 'Author' : 'Artist'}</label>
          <input value={editedArtist} onChange={e => setEditedArtist(e.target.value)} placeholder={selectedFormat === 'mp4' ? 'Enter author' : 'Enter artist'} className='w-full bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-cyan-500/50 focus:bg-black/40 transition-all placeholder-gray-600' />
        </div>
        {selectedFormat !== 'mp4' && (
