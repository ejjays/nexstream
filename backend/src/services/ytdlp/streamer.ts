  ...
  if (isMerging) {
    if (isWebm) {
      ...
      if (shouldCopy) {
         ...
         args.push("--downloader", "ffmpeg", "--downloader-args", `ffmpeg:-c:v libx264 -preset ${preset} -threads 0 -crf 24 -maxrate 3000k -bufsize 6000k -c:a aac -b:a 128k -bsf:a aac_adtstoasc -f mp4 -movflags frag_keyframe+empty_moov+default_base_moof -frag_duration 1000000`);
      }
    }
  }
