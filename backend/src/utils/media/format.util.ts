...
export function estimateFilesize(format: RawFormat, duration: number): number {
  ...
  const bitrate = Number(format.bitrate) || (format.tbr ? Number(format.tbr) * 1024 : 0);
  if (bitrate && duration) {
...
function resolveResolution(format: RawFormat): { resolution: string; height: number } {
...
function resolveCodecs(format: RawFormat) {
  const acodec = String(format.acodec || (format.vcodec && format.vcodec !== 'none' ? 'yes' : 'none'));
  const vcodecStr = String(format.vcodec || '');
  const isMuxed = format.isMuxed || format.is_muxed || (vcodecStr !== 'none' && vcodecStr !== '' && acodec !== 'none' && acodec !== '');
...
function calculateFinalSize(format: RawFormat, duration: number, vcodec: string) {
  let estimatedSize = estimateFilesize(format, duration);
  const extension = String(format.ext || format.extension || 'mp4');
  
  if (extension === 'webm' || vcodec.includes('av01') || vcodec.includes('vp9')) {
      estimatedSize *= 1.35;
  }
...
export function processVideoFormats(info: { duration?: number; formats?: RawFormat[]; streaming_data?: { formats?: RawFormat[]; adaptive_formats?: RawFormat[] } }): Format[] {
  ...
  for (const format of processed) {
    ...
    if (!existing || (format.isMuxed && !existing.isMuxed) || (format.isMuxed === existing.isMuxed && (format.filesize || 0) > (existing.filesize || 0))) {
...
export function processAudioFormats(info: { formats?: RawFormat[]; streaming_data?: { formats?: RawFormat[]; adaptive_formats?: RawFormat[] } }): Format[] {
  ...
  const processed = formats
    .filter((format: RawFormat) => {
      ...
      const isAudioOnly = 
        ((acodec !== "none" && acodec !== "" && (vcodec === "none" || vcodec === "")) ||
        (formatId.includes('audio')) ||
        (extension === 'm4a' && (vcodec === "none" || vcodec === "")) ||
        (acodec !== "" && vcodec === "")) && extension !== 'webm';
    ...
    .map((format: RawFormat): Format | null => {
      ...
      const formatId = String(format.formatId || format.format_id || format.itag);
      if (extension === 'mp4' || extension === 'm4a' || String(format.acodec).includes('mp4a') || formatId.includes('m4a')) {
          extension = 'm4a';
      }
