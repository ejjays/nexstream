...
async function waitForServer(url: string, serverProcess: import('node:child_process').ChildProcess, timeout = 30000): Promise<void> {
...
async function runE2E() {
  ...
  const server = spawn('node', ['./scripts/termux-shim.js'], { stdio: 'inherit', shell: true, detached: true });
  ...
  try {
    ...
    await page.goto('http://localhost:5000/health', { waitUntil: 'networkidle2' });
