...
exports.seedIntelligence = async (req, res) => {
    const {
        url,
        id: clientId = "admin-seeder"
    } = req.query;
    if (!url) return res.status(400).json({
        error: "No Spotify Artist/Album URL provided"
    });
    ...
    try {
        ...
        if (!tracks || tracks.length === 0)
            throw new Error("No tracks found in the provided link.");
                ...
                try {
                    ...
                    const result = await resolveSpotifyToYoutube(trackUrl, [], (status, progress, data) => {
                        if (clientId) sendEvent(clientId, {
                            status: "seeding",
                            subStatus: `Processing: "${track.name} by ${track.artists?.[0]?.name || "Unknown"}"`,
...
async function handleSpotifyRequest(videoURL, cookieArgs, clientId) {
    if (clientId) sendEvent(clientId, {
        ...
        details: "UPLINK: SYNCHRONIZING_METADATA_STREAM"
    });
...
exports.convertVideo = async (req, res) => {
    ...
    if (format === "mp3") {
        ...
        if (clientId) sendEvent(clientId, {
            status: "initializing",
            progress: 5,
            subStatus: "Lightning Engine Active: Starting Stream...",
            details: "HYBRID_ENGINE: INSTANT_MP3_DISPATCH"
        });
        ...
        (async () => {
            try {
                ...
                const videoProcess = streamDownload(streamURL, {
                    format,
                    formatId
                }, cookieArgs, info);
                ...
                videoProcess.on("close", code => {
                    console.log(
                        `[Lightning MP3] Process closed with code ${code}. Total bytes sent: ${totalBytesSent}`
                    );
                    if (code !== 0 && totalBytesSent > 0 && clientId) sendEvent(clientId, {
                        status: "error",
    ...
    try {
        ...
        const videoProcess = streamDownload(targetURL, {
            format,
            formatId
        }, cookieArgs, info);
        let totalBytesSent = 0;
        ...
        videoProcess.on("close", code => {
            console.log(
                `[Convert] Process closed with code ${code}. Total bytes sent: ${totalBytesSent}`
            );
            if (code !== 0 && totalBytesSent > 0 && clientId) sendEvent(clientId, {
                status: "error",
                message: "Stream interrupted"
...
exports.seedIntelligence = async (req, res) => {
    ...
    try {
        ...
        if (!tracks || tracks.length === 0) {
            throw new Error(
                "No tracks found in the provided link. Ensure it is a valid Spotify Track, Album, or Artist URL."
            );
        }
