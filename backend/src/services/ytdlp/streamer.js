...
function streamDownload(url, options, cookieArgs = [], preFetchedInfo = null) {
  ...
  (async () => {
    try {
      ...
      if (extractor && typeof extractor.getStream === 'function' && info.extractor_key !== 'youtube' && info.extractor_key !== 'tiktok') {
      ...
      // INSTANT PATH: If have pre-fetched info JSON, load directly to skip scraping
      if (info && info.info_json_path) {
          console.log(`[Streamer] Using Instant-JSON Path: ${info.info_json_path}`);
          args.push("--load-info-json", info.info_json_path);
      } else {
          args.push(url);
      }
